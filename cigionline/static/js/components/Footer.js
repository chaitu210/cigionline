...
const Footer = ({ slide, socialIcons, contentOpacity, changeStyle, changeSocialStyle }) => {
  const originUrl = window.location.origin;
  const location = window.location.href;
  const lightBackgroundSlugs = ['messageslidepage', 'tabbedslidepage', 'outputsandactivitiesslidepage'];
  const noCameraSlides = ['outputsandactivitiesslidepage', 'timelineslidepage', 'tabbedslidepage'];

  return (

            <div className={"footer clearfix show-for-large"  + (lightBackgroundSlugs.indexOf(slide.type)>-1 ? ' footer-dark' : '')}>
